# Better Agent Terminal - Application Flow
...
```
Workspace activated (WorkspaceView useEffect)
   │
   ├─► Create "Main" terminal (type: claude-code)
   │   ├─► workspaceStore.addTerminal(workspaceId, 'claude-code')
   │   └─► tauriAPI.pty.create({ id, cwd, type })
   │       │
   ...
   │
   └─► Create "Terminal 1" (type: terminal)
       └─► Same flow as above
```
